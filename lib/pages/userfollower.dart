...
class UserFollower extends StatefulWidget {
  UserFollower(this.user, this.myUser);
  final String user;
  final String myUser;
  static const route = '/userfollower';
  @override
...
class _UserFollowerState extends State<UserFollower> {
  ...
  getFollowInfo([int? page]) async {
    if (page != null) {
      ...
      try {
        response =
            await followerService.getUserFollowers(widget.user, widget.myUser);
        return response;
      } catch (e) {
        print(e);
      ...
      var response;
      try {
        response = await followerService.getUserFollowers(
  ...
  Widget _noFollowers() {
    return Expanded(
      child: Column(
        children: [
          Text(
            '${widget.user} has no followers yet',
            textAlign: TextAlign.center,
            style: TextStyle(
                fontWeight: FontWeight.bold,
                fontSize: 30,
      ...
      child: Column(
        children: [
          ...
          Padding(
            ...
            child: Text('There was an error getting ${widget.user} followers',
                textAlign: TextAlign.center,
                style: TextStyle(
                    fontWeight: FontWeight.bold,
                    fontSize: 30,
